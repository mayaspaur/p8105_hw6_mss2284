...
In proposing a regression model for birthweight, I explore a data-driven model-building process using backward selection, whereby variables that are statistically significant remain the model.
...
boot_straps_1 =
  boot_straps %>% 
  unnest(results) %>%
  ggplot(aes(x = r.squared)) + geom_density() + labs(x = "r^2")
...
boot_straps_2_plot =
  boot_straps_2 %>%
  ggplot(aes(x = log_term)) + geom_density() + labs(x = "log(β^0∗β^1)")
